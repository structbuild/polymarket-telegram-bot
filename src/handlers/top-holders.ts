...
export function buildMarketDetailKeyboard(
  ...
  return kb
    ...
    .row()
    .text("👥 Holders", `ma:${cacheId}:holders`)
    .text("🏆 Traders", `ma:${cacheId}:traders`)
    .row()
    .text("🧾 Trades", `ma:${cacheId}:trades`)
    .text("📖 Book", `ob:${cacheId}`)
    .row()
    .text("📈 Holders Hist", `ma:${cacheId}:holderhist`)
    .text("📊 Outcomes", `ma:${cacheId}:outcomes`)
    .row()
    .text("⚡ Jumps", `pj:${cacheId}`)
    .row()
    .text("✕ Close", "close")
    .danger();
}
